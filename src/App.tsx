...
function Background3D({
  ...
  theme: "cosmic" | "neon" | "matrix" | "lounge";
}) {
  ...
  return (
    <div
      ref={containerRef}
      className="absolute inset-0 w-full h-full -z-20 overflow-hidden opacity-90 pointer-events-none"
    />
  );
}
...
export default function App() {
  ...
  const [selected3DTheme, setSelected3DTheme] = useState<
    "cosmic" | "neon" | "matrix" | "lounge"
  >("cosmic");
  const [isChatSlideoutOpen, setIsChatSlideoutOpen] = useState(false);
  const [isProfileVerified, setIsProfileVerified] = useState(false);
  const [verificationStep, setVerificationStep] = useState<
    "idle" | "pose_prompt" | "scanning" | "verified"
  >("idle");
  ...
  const [messages, setMessages] = useState<
    Array<{ id: string; sender: string; text: string; time: number }>
  >([]);
  ...
  const setupMedia = async () => {
    try {
      ...
      // If we are no longer in matching/active state by the time media resolves, shut it down.
      if (isTransitioningRef.current === false && !chatMode) {
      ...
      console.warn(
        "Camera/Mic not permitted. Generating virtual camera overlay.",
        err,
      );
    ...
    const intervalId = setInterval(() => {
      ...
      if (partnerId === "sim_sam") {
        ...
        for (let i = 0; i < 480; i++) {
          const y =
            220 +
            Math.cos(i * 0.02 - angle) * 30 +
            Math.sin(i * 0.07 + angle) * 10;
          if (i === 0) ctx.moveTo(i, y);
          else ctx.lineTo(i, y);
        }
        ctx.stroke();
    ...
    if (localVideoRef.current && localStream) {
      if (localVideoRef.current.srcObject !== localStream) {
        ...
        localVideoRef.current.play().catch((e) => {
          if (e.name !== "AbortError") console.warn("Local play blocked:", e);
        });
      }
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.status === "active" && data.room) {
            ...
            if (status === "active") {
              addSystemMessage(
                "Partner skipped or disconnected. Finding a new partner...",
              );
              cleanupWebRTC();
            }
            setStatus("matching");
  ...
  const fetchMessages = async () => {
    try {
      ...
      setPartnerTyping((prev) => {
        if (!prev && data.partnerTyping) {
          scrollToBottom();
        }
        return data.partnerTyping || false;
      });
    } catch (err) {
  ...
  const startChatMatchmaking = async (
    mode: "video" | "text",
    forceBypassAgeCheck = false,
  ) => {
    if (sysBanned) return;
  ...
  return (
    <div className="relative min-h-[100dvh] flex flex-col justify-between text-zinc-100 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#09091b] via-[#020206] to-[#000000] font-sans antialiased overflow-x-hidden select-none">
      ...
      {sysBanned && (
        <div className="fixed inset-0 bg-[#020206]/95 flex flex-col items-center justify-center p-4 z-50 text-center">
          <ActionCard className="max-w-md bg-zinc-950/80 border border-red-500/20 p-6 rounded-2xl shadow-2xl">
            ...
            <h1 className="text-lg font-mono font-bold text-red-400 uppercase tracking-widest mb-2">
              Access Suspended
            </h1>
            <p className="text-zinc-405 text-xs mb-4 leading-relaxed">
              Your session has been terminated by our moderation patrol system
              due to behavior report validation.
            </p>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-40 animate-fade-in">
          <ActionCard className="w-full max-w-md p-6 relative bg-zinc-950/80 border border-white/10 shadow-2xl rounded-2xl">
            ...
            <div className="text-center mb-5">
              ...
              <h1 className="text-xl font-bold font-mono tracking-tight mt-3 text-zinc-100">
                Age Verification Gate
              </h1>
              <p className="text-[10px] text-zinc-400 mt-1">
                Please confirm you meet the age requirements to enter{" "}
                {pendingChatMode === "video" ? "Video Call" : "Text Chat"}.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-bold uppercase tracking-wider text-zinc-400 block font-mono">
                  Confirm Alias / Nickname
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-3 p-3.5 bg-black/40 border border-white/5 rounded-xl">
                ...
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  ...
                  <span className="text-[11px] text-zinc-300 leading-relaxed font-sans">
                    ...
                    and I legally consent and agree to the{" "}
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                ...
                <p className="text-[10px] text-zinc-500 font-serif leading-relaxed italic border-t border-white/5 pt-2">
                  JustChat requires all participants to be 18+. Any explicit,
                  harassing, or synthetic activities will result in immediate
                  state bans.
                </p>
              </div>
      ...
      {!sysBanned && chatMode === null && (
        <div
          className="flex-1 w-full max-w-4xl mx-auto px-4 py-8 flex flex-col justify-center items-center gap-8 min-h-0 animate-fade-in"
        ...
        >
          <div className="text-center space-y-2 max-w-lg">
            ...
            <h1 className="text-5xl sm:text-6xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 font-mono drop-shadow-[0_4px_16px_rgba(168,85,247,0.5)]">
              JustChat
            </h1>
            <p className="text-xs sm:text-sm text-[#8e9fdd] font-medium max-w-md mx-auto drop-shadow-sm">
              Dynamic matching on encrypted streams. Pick a connection protocol
              and align tag parameters to peer with the space.
            </p>
          </div>
          ...
          <ActionCard
            ...
            className="w-full max-w-2xl p-5 sm:p-6 shadow-[0_8px_32px_rgba(59,130,246,0.3)] border border-white/20 backdrop-blur-md relative overflow-hidden rounded-3xl text-blue-50"
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/5 border border-white/20 p-3.5 rounded-xl backdrop-blur-md shadow-inner shadow-white/5">
              <div className="space-y-0.5">
                <span className="text-[9px] font-bold text-blue-200 uppercase font-mono tracking-wider">
                  Your Alias Profile
                </span>
            ...
            <div className="space-y-3 relative z-10">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-blue-200 uppercase tracking-widest flex items-center gap-1.5 font-mono">
                  <Tag className="w-3.5 h-3.5 text-teal-400" /> Match Interests
                  & Tags
                </span>
                <span className="text-[9px] text-indigo-300 font-mono">
                  Shared intersections prioritized
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 min-h-[40px] p-2.5 bg-black/20 border border-white/20 rounded-xl backdrop-blur-md shadow-inner shadow-white/5">
                ...
                {interests.length === 0 && (
                  <span className="text-[10px] text-blue-200/70 italic font-mono p-1">
                    No interest tags configured. Swiping global pools.
                  </span>
                )}
              </div>
            ...
            <div className="space-y-3 pt-2 relative z-10">
              <span className="text-[10px] font-bold text-indigo-300 uppercase tracking-widest block font-mono">
                Choose Matching Protocol
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <h3 className="text-sm font-bold font-mono text-white mb-1 relative z-10 drop-shadow-md">
                    Start Video Chat
                  </h3>
                  <p className="text-[10px] text-blue-100 leading-normal font-sans relative z-10">
                    Match with human peers over an interactive encrypted
                    streaming video channel feed.
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="text-[10px] text-blue-100 leading-normal font-sans relative z-10">
                    Plain, elegant messaging chat space. Communicate safely
                    using text log transmissions.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between pt-4 border-t border-white/20 relative z-10 shadow-t">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[9px] text-blue-200 font-mono font-bold uppercase tracking-wider">
                  3D Hologram:
                </span>
                <div className="flex gap-1.5 p-1 bg-black/20 rounded-xl backdrop-blur-sm border border-white/10 shadow-inner">
                  {(["cosmic", "neon", "matrix", "lounge"] as const).map(
                    (themeName) => (
                      <button
                        key={themeName}
              ...
              <button
                onClick={() =>
                  setVerificationStep(
                    isProfileVerified ? "verified" : "pose_prompt",
                  )
                }
              ...
              >
                <CheckCircle2
                  className={`w-3.5 h-3.5 ${isProfileVerified ? "text-emerald-400" : "text-blue-300"}`}
                />
                <span>
                  {isProfileVerified ? "VERIFIED HUMAN" : "PROOF HUMAN POSE"}
                </span>
              </button>
            </div>
          </ActionCard>
      ...
      {isAgeVerified && !sysBanned && chatMode !== null && (
        <main className="flex-1 w-full max-w-7xl mx-auto p-3 flex flex-col lg:grid lg:grid-cols-12 gap-3 relative animate-fade-in min-h-0">
          ...
          {status === "matching" && (
            <div
              className="lg:col-span-12 flex flex-col items-center justify-center gap-6 flex-1 text-center animate-fade-in"
              id="queue_loading_viewport"
            >
              ...
              <div className="space-y-2 max-w-md">
                <h3 className="text-lg font-bold font-mono tracking-tight text-teal-400 animate-pulse">
                  Connecting to partner...
                </h3>
                <p className="text-xs text-[#6f6f7c] font-medium">
                  Searching for available matching peers for{" "}
                  {chatMode === "video" ? "Video Call" : "Text Chat"}...
                </p>
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono">
                  Running secure tag overlap scanners
                </p>
                ...
                {interests.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                    {interests.map((i) => (
                      <span
                        key={i}
                        className="text-[9px] bg-black/40 border border-white/5 text-zinc-400 px-2 py-0.5 rounded-full font-mono font-medium"
                      >
                        #{i}
                      </span>
                    ))}
              ...
              <div className="flex flex-col gap-2 items-center">
                <p className="text-[10px] text-zinc-450 font-mono">
                  Instant mock matches will activate if queue is quiet.
                </p>
                <button
              ...
              {chatMode === "video" && (
                <section className="col-span-full lg:col-span-12 flex flex-col gap-3 shrink-0 relative">
                  <div className="relative flex flex-col lg:grid lg:grid-cols-2 lg:grid-rows-1 gap-2 h-[45vh] lg:h-[65vh]">
                    ...
                    <ActionCard
                      className={`bg-zinc-950/80 border border-white/10 p-3 relative flex flex-col justify-between shadow-2xl overflow-hidden min-h-0 min-w-0 lg:col-span-1 rounded-xl shrink-0 h-full max-lg:absolute max-lg:inset-0 max-lg:z-0 lg:flex-1`}
                    >
                      ...
                      {partner && (
                        <div className="absolute top-2.5 right-2.5 z-10">
                          <div className={`flex gap-1.5 max-lg:hidden`}>
                            {(partner.interests || [])
                              .slice(0, 2)
                              .map((i: string, id: number) => (
                                <span
                                  key={id}
                                  className="bg-black/60 border border-white/10 text-zinc-400 text-[8px] px-1.5 py-0.5 font-bold font-mono rounded"
                                >
                                  #{i}
                                </span>
                              ))}
                      ...
                      >
                        {remoteStream ? (
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-650 text-[10px] font-mono text-center p-2">
                            ...
                            <p className="text-[8px] text-zinc-550 leading-relaxed mt-1 font-sans hidden sm:block">
                              Wait for peer's audio & video stream signal
                              matching.
                            </p>
                          </div>
                        )}
                      ...
                      <div
                        className={`w-full flex items-center justify-between mt-1 text-[9px] font-mono text-zinc-500 max-lg:hidden`}
                      >
                        <span className="truncate">
                          {partner
                            ? `Connecting: ${partner.name}`
                            : "Handshake..."}
                        </span>
                    ...
                    <ActionCard
                      className={`bg-zinc-950/80 border border-white/10 p-3 relative flex flex-col justify-between shadow-2xl overflow-hidden min-h-0 min-w-0 lg:col-span-1 rounded-xl shrink-0 h-full max-lg:absolute max-lg:bottom-2 max-lg:right-2 max-lg:w-[120px] max-lg:h-[160px] max-lg:z-50 lg:flex-1`}
                    >
                      <div
                        className={`absolute top-2.5 left-2.5 z-10 flex items-center gap-1 max-lg:hidden`}
                      >
                        <span className="px-1.5 py-0.5 bg-black/60 border border-white/10 text-zinc-350 text-[8px] font-bold font-mono rounded uppercase">
                          You
                        </span>
                      ...
                      <div
                        className={`absolute top-2.5 right-2.5 z-10 max-lg:hidden`}
                      >
                      ...
                      >
                        ...
                        {(!cameraActive || !localStream) && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-650 text-[10px] font-mono">
                            <VideoOff
                              className={`w-6 h-6 mb-1 text-zinc-600 max-lg:w-4 max-lg:h-4 max-lg:mb-0`}
                            />
                  ...
                  <div className="bg-[#141414] border border-[#1F1F1F] p-3 rounded flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 relative z-[90]">
                    <div className="flex flex-wrap gap-2 items-center justify-center sm:justify-start">
                      <button
                        onClick={handleEndMatch}
                        className="px-4 py-2 bg-red-950/45 hover:bg-red-955/65 border border-red-900/40 text-red-400 font-bold rounded text-xs font-mono transition inline-flex items-center gap-1 animate-fade-in cursor-pointer flex-1 sm:flex-none justify-center"
                      >
                      ...
                      <button
                        ...
                        className="px-5 py-2 bg-zinc-200 hover:bg-white text-zinc-950 font-bold rounded text-xs font-mono transition inline-flex items-center gap-1 animate-fade-in cursor-pointer flex-1 sm:flex-none justify-center"
                      >
                      ...
                      {chatMode === "video" && (
                        <div className="flex gap-2 lg:hidden">
                          <button
                            onClick={toggleMic}
                            className={`p-2 rounded border transition cursor-pointer flex items-center justify-center flex-1 ${micActive ? "bg-black/60 border-white/15 text-zinc-300" : "bg-red-950/30 border-red-900/40 text-red-400"}`}
                            title={
                              micActive
                                ? "Mute Microphone"
                                : "Unmute Microphone"
                          ...
                          <button
                            onClick={toggleCamera}
                            className={`p-2 rounded border transition cursor-pointer flex items-center justify-center flex-1 ${cameraActive ? "bg-black/60 border-white/15 text-zinc-300" : "bg-red-950/30 border-red-900/40 text-red-400"}`}
                            title={cameraActive ? "Mute Video" : "Unmute Video"}
                          >
                            {cameraActive ? (
                      ...
                      <button
                        onClick={() =>
                          setIsChatSlideoutOpen(!isChatSlideoutOpen)
                        }
                        className="px-4 py-2 bg-indigo-950/40 hover:bg-indigo-900/60 border border-indigo-500/30 text-indigo-400 font-bold rounded text-xs font-mono transition inline-flex items-center gap-1 animate-fade-in cursor-pointer lg:hidden"
                      >
                    ...
                    <div className="flex items-center gap-1.5 bg-[#0A0A0A] p-1.5 rounded border border-[#1F1F1F] justify-center sm:justify-end mx-auto sm:mx-0 min-w-0">
                      <span className="text-zinc-500 font-semibold text-[9px] uppercase tracking-wider px-1 font-mono">
                        Theme:
                      </span>
                ...
                <div className="bg-[#141414] border border-[#1F1F1F] rounded flex-1 flex flex-col overflow-hidden shadow-sm h-full">
                  ...
                  <div className="bg-[#0F0F0F] border-b border-[#1F1F1F] p-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      {(partner?.interests || []).map((i) => (
                        <span
                          key={i}
                          className="text-[9px] bg-[#0A0A0A] border border-[#1F1F1F] text-zinc-400 px-1.5 py-0.2 rounded font-mono hidden sm:inline-block"
                        >
                          #{i}
                        </span>
                      ))}
                  ...
                  <div className="flex-1 overflow-y-auto p-3.5 space-y-3 bg-[#0D0D0D]">
                    {messages.length === 0 && !partnerTyping && (
                      <div className="flex flex-col items-center justify-center text-center p-5 text-zinc-650 space-y-1.5 min-h-[150px]">
                        <MessageSquare className="w-7 h-7 text-zinc-800" />
                        <div className="space-y-0.5 max-w-xs">
                          <p className="font-bold text-[10px] font-mono text-zinc-500 uppercase">
                            Handshake Channel active
                          </p>
                          <p className="text-[9px] text-zinc-650 leading-normal font-sans">
                            Session text transmissions are anonymous, and
                            strictly subject to local community trust controls.
                            Say hello!
                          </p>
                        </div>
                      </div>
                    ...
                    {messages.map((m) => {
                      ...
                      return (
                        <div
                          key={m.id}
                          className={`flex flex-col max-w-[85%] ${isSelf ? "ml-auto items-end" : "mr-auto items-start animate-fade-in"}`}
                        >
                          <div className="text-[8px] text-zinc-600 font-mono mb-0.5 px-0.5">
                            {isSelf ? "You" : partner?.name || "Partner"} •{" "}
                          ...
                          <div
                            className={`px-2.5 py-1.5 rounded text-xs leading-relaxed font-mono ${
                              isSelf
                  ...
                  <form
                    ...
                    className="bg-[#0F0F0F] border-t border-[#1F1F1F] p-2 flex gap-1.5"
                  >
                    <input
                      type="text"
          ...
          <div className="w-full max-w-sm bg-[#141414] border border-[#1F1F1F] rounded p-5 relative">
            ...
            <div className="text-center mb-4 space-y-1">
              ...
              <h2 className="text-sm font-bold font-mono text-zinc-100">
                Prove Human Presence
              </h2>
              <p className="text-[10px] text-zinc-400">
                Perform this random hand gesture clearly in front of your
                camera.
              </p>
            </div>
            ...
            {verificationStep === "scanning" && (
              <div className="p-4 text-center space-y-3">
                ...
                <div className="space-y-0.5">
                  <p className="font-bold text-xs font-mono text-zinc-200">
                    Analyzing Face & Hand Gesture Pose...
                  </p>
                  <p className="text-[8px] text-zinc-500 font-mono uppercase tracking-wider">
                    Neural pose checking
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-[#0A0A0A]/85 backdrop-blur-sm flex items-center justify-center p-4 z-40">
          <div className="w-full max-w-md bg-[#141414] border border-[#1F1F1F] rounded p-5 relative">
            ...
            <div className="text-center mb-4 space-y-1">
              ...
              <h2 className="text-sm font-bold font-mono text-zinc-100">
                Send Trust violation
              </h2>
              <p className="text-[10px] text-zinc-450">
                Please report harassment or inappropriate actions.
              </p>
            </div>
            ...
            {showReportSuccess ? (
              <div className="p-3 text-center space-y-2 animate-fade-in">
                ...
                <p className="text-[10px] text-zinc-400 font-sans">
                  Checking session conversation logs manually. Resetting peer
                  room...
                </p>
              </div>
            ) : (
              <div className="space-y-4 font-sans">
                <div className="bg-[#0A0A0A] p-2.5 rounded border border-[#1F1F1F] space-y-0.5">
                  <span className="text-[8px] text-zinc-520 uppercase font-mono block">
                    Suspect Session key:
                  </span>
                  <span className="text-[10px] text-zinc-350 font-mono block break-all font-semibold select-all">
                    {partner?.id || "Unknown"}
                  </span>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-zinc-450 font-mono uppercase block">
                    Violation Reason
                  </label>
                  <select
                    value={reportReason}
                  ...
                  >
                    <option value="Inappropriate stream">
                      Inappropriate stream feed or action
                    </option>
                    <option value="Harassment">
                      Harassment or abusive speech
                    </option>
                    <option value="Bot/Spam">
                      Synthetic Bot / advertising links
                    </option>
                    <option value="Minor profile">
                      Suspected under-aged account
                    </option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-zinc-455 font-mono uppercase block">
                    Extra description
                  </label>
                  <textarea
                    rows={3}
      ...
      <footer className="bg-[#0F0F0F] border-t border-[#1F1F1F] px-4 py-4 flex flex-col md:flex-row items-center justify-between gap-4 text-[10px] text-zinc-400 font-mono">
        ...
        <div>
          <span>
            &copy; {new Date().getFullYear()} JustChat Space • Preserving
        ...
        >
          <ActionCard className="w-full max-w-lg p-6 relative bg-zinc-950/95 border border-zinc-800 shadow-2xl rounded-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs text-zinc-300 leading-relaxed font-sans">
              <p>
                <strong>JustChat</strong> is a next-generation real-time peering
                lounge built to safely match passionate coders, musicians,
                artists, and creators globally.
              </p>

              <div className="p-3 bg-white/5 border border-white/10 rounded-xl space-y-1.5 font-mono text-[11px]">
                <h4 className="font-bold text-teal-400 uppercase">
                  Core Architecture details
                </h4>
                <ul className="list-disc pl-4 space-y-1 text-zinc-400">
                  <li>
                    <strong>Instant Match Engine:</strong> Algorithmic pairing
                    based on tag intersects.
                  </li>
                  <li>
                    <strong>Authentic Connections:</strong> Direct
                    human-to-human chats with no chatbots, AI bots, or fake
                    filler entries.
                  </li>
                  <li>
                    <strong>Custom Holographic Vibe Scenes:</strong> 3D canvas
                    rendering supporting Cosmic, Neon, Lounge, and Matrix
                    environments.
                  </li>
                  <li>
                    <strong>P2P Media Protocols:</strong> Encrypted WebRTC
                    streams.
                  </li>
                </ul>
              </div>
              ...
              <p>
                Our mission is to help restore genuine, instant, friction-free
                social micro-conversations without the bloat, ads, trackers, or
                endless algorithms of typical platforms.
              </p>
          ...
          <ActionCard className="w-full max-w-lg p-6 relative bg-zinc-950/95 border border-zinc-800 shadow-2xl rounded-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs text-zinc-300 leading-relaxed font-sans">
              <p className="font-semibold text-zinc-200">
                Welcome to JustChat. By using this platform, you commit to
                respecting the community standard rules, terms, and guidelines:
              </p>

              <ol className="list-decimal pl-4 space-y-3 text-zinc-400 font-mono text-[11px]">
                <li>
                  <strong className="text-zinc-205">
                    Age Check requirements:
                  </strong>
                  <br />
                  You must be at least 18 years of age. Underaged users are
                  strictly prohibited and will be reported to moderation
                  authorities.
                </li>
                <li>
                  <strong className="text-zinc-205">
                  ...
                  <br />
                  Harassment, bullying, toxic speeches, vulgar displays, or
                  non-consensual multimedia broadcasts are forbidden.
                </li>
                <li>
                  <strong className="text-zinc-205">Service Suspension:</strong>
                  <br />
                  Our system automatically flags high-frequency skips or
                  reports. Valid violation reports trigger immediate hardware IP
                  bans.
                </li>
                <li>
                  <strong className="text-zinc-205">
                    P2P Security responsibility:
                  </strong>
                  <br />
                  Stream data is encrypted and negotiated peer-to-peer. JustChat
                  does not cache transmission streams or logs. You are solely
                  responsible for interactions you initiate.
                </li>
              </ol>

              <div className="p-3 bg-red-950/20 border border-red-900/40 rounded-xl leading-relaxed text-red-300 text-[10px] font-mono">
                SAFETY PROTOCOL: Violating the terms will terminate your session
                and black-list your session signatures from joining again.
              </div>
